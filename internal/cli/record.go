...
working directory (or the directory given by --output).`,
	...
	RunE: withDevice(recordRunE),
}

func recordRunE(cmd *cobra.Command, args []string, runner adb.Runner, serial string) error {
	autoName := fmt.Sprintf("video_%s.mp4", time.Now().Format("20060102_150405"))
	localPath, err := resolveCapturePath(cmd, args, autoName, ".mp4")
	if err != nil {
		return err
	}
