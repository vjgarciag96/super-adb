...
- [ ] `sadb shot [<path>]` accepts an optional positional argument
- [ ] `sadb record [<path>]` accepts an optional positional argument
- [ ] When `<path>` is provided it is used as the full local file path for the pull destination
- [ ] When `<path>` is omitted behaviour is identical to today (auto-generated name under cwd or `--output` dir)
- [ ] `--output` and positional `<path>` are mutually exclusive; passing both returns a clear error
- [ ] `RunPhoto` / `RunVideo` signatures updated to accept the resolved local path rather than computing it internally (so the CLI layer owns path resolution and the core functions remain testable)
- [ ] Tests cover: explicit path used; auto-generated fallback; mutual-exclusion error
