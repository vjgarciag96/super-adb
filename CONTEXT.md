...
**Shell Completion**:
The mechanism by which a shell (zsh, bash, fish, PowerShell) suggests valid subcommands, flags, and arguments when the user presses Tab. sadb bundles its own completion logic via Cobra's built-in completion system — users activate it once per shell, after which it works automatically.
_Avoid_: autocomplete, tab complete, auto-suggest

**Completion Script**:
The shell-specific script generated by `sadb completion <shell>` that teaches the user's shell what sadb accepts. Distributed automatically via the Homebrew formula; also obtainable manually via `sadb completion install`.
_Avoid_: completion file, tab file
